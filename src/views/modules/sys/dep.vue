...
<script>
import AddOrUpdate from './dep-add-or-update'
export default {
    data(){
        return {
            dataForm:{
    ...
    methods: {
        getDataList(){
            ...
            this.$http({
                url: this.$http.adornUrl('/sys/dep/page'),
                method: 'get',
      ...
      addOrUpdateHandle (id) {
        ...
        // 删除
        deleteHandle(id){
            var depIds = id ? [id] : this.dataListSelections.map(item=>{
                return item.id
            })
            this.$confirm(`确定对[id=${depIds.join(',')}]进行[${id ? '删除' : '批量删除'}]操作?`, '提示', {
            ...
            type: 'warning'
                ...
                this.$http({
                    ...
                    method: 'post',
                    data: this.$http.adornData(depIds, false)
                }).then(({data}) => {
                    if (data && data.code === 0) {
                    this.$message({
                        message: '操作成功',
